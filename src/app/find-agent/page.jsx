  ...
  useEffect(() => {
    ...
    if (filter.addressSearch || filter.nameSearch) {
      const filtered = pageData.filter((agent) => {
        const addressMatch = !filter.addressSearch ||
          formatText(agent?.address || "").includes(formatText(filter.addressSearch)) ||
          formatText(agent?.BillingStreet || "").includes(formatText(filter.addressSearch)) ||
          formatText(agent?.Billingcity || "").includes(formatText(filter.addressSearch)) ||
          formatText(agent?.BillingState || "").includes(formatText(filter.addressSearch));

        const nameMatch = !filter.nameSearch ||
          formatText(agent?.name || "").includes(formatText(filter.nameSearch)) ||
          formatText(agent?.agent_name || "").includes(formatText(filter.nameSearch)) ||
          formatText(agent?.RecruitmentAgentName || "").includes(formatText(filter.nameSearch)) ||
          formatText(agent?.RepresentativeContactName || "").includes(formatText(filter.nameSearch));
            ...
            <div className="mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredData.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="p-6 bg-primary-orange/5 rounded-lg shadow-sm border border-gray-200">
                      <div className="flex flex-col gap-4">
                        {/* Name Section */}
                        <div>
                          <p className="text-lg font-bold text-gray-800">{item.name || "N/A"}</p>
                          <p className="text-sm text-gray-600 mt-1">
                            <span className="font-semibold">Representative:</span> {item?.agent_name || "N/A"}
                          </p>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <a
                                ...
                                className={`text-sm ${item?.phone ? "text-blue-600 hover:underline" : "text-gray-500"}`}
                              >
                                {item?.phone || "N/A"}
                              </a>
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <a
                                ...
                                className={`text-sm ${item?.email ? "text-blue-600 hover:underline" : "text-gray-500"}`}
                              >
                                {item?.email || "N/A"}
                              </a>
                            </div>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <a
                                href={item?.address ? `https://www.google.com/maps/search/${encodeURIComponent(item.address)}` : undefined}
                                target="_blank"
                                rel="noopener noreferrer"
                                className={`text-sm ${item?.address ? "text-blue-600 hover:underline" : "text-gray-500"}`}
                              >
                                {item?.address || "N/A"}
                              </a>
                            </div>
